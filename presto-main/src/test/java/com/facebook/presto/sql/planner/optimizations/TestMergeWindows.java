...
public class TestMergeWindows extends BasePlanTest
{
        ...
        assertUnitPlan(sql,
                anyTree(
                        window(specificationA,
                                ...
                                project(ImmutableList.of("ONE", expression("1"), "ZERO", expression("0.0")),
                                        window(specificationA,
                                                ImmutableList.of(
                                                functionCall("sum", COMMON_FRAME, ImmutableList.of(QUANTITY_ALIAS))),
                                                any())))));
    ...
    */
    ...
    public void testNotMergeAcrossJoinBranches()
    {
        ...
        leftBuilder.put(ORDERKEY_ALIAS, "orderkey");
        leftBuilder.put("PARTKEY", "partkey");
        leftBuilder.put(SUPPKEY_ALIAS, "suppkey");
        leftBuilder.put(QUANTITY_ALIAS, "quantity");
        leftBuilder.put(SHIPDATE_ALIAS, "shipdate");

        PlanMatchPattern leftTableScan = tableScan("lineitem", leftBuilder.build());
